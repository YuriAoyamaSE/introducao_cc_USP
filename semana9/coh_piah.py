...
def calcula_assinatura(texto):
    ...
    palavras = []
    for item in frases:
        palavras.extend(separa_palavras(item))

    wal = sum([len(item) for item in palavras]) / len(palavras)
    ttr = n_palavras_diferentes(palavras) / len(palavras)
    hlr = n_palavras_unicas(palavras) / len(palavras)
    sal = sum([len(item) for item in sentencas]) / len(sentencas)
    sac = len(frases) / len(sentencas)
    pal = sum([len(item) for item in frases]) / len(frases)
...
def avalia_textos(textos, ass_cp):
    ...
    for texto in textos:
        ...
        similaridades.append(compara_assinatura(ass_txt, ass_cp))
    texto_suspeito = similaridades.index(min(similaridades)) + 1
